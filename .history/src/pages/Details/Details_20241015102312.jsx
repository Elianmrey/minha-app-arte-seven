...
export default function Details({ cardType }) {
    ...
    if (err) return (<div className={ cardTypeerror}>{err}</div>);
    if (!info) return (<div>Item não encontrado</div>);
    
    return (
        <div className="container">

            <img className={ } src={`${urlBase + info.backdrop_path}`} />
            
            <h1>{info.title}</h1>
            <p>{info.overview}</p>
            <p>Nota: {info.vote_average}</p>
            {cardType === "tvShows" ? <div><p>Temporadas: {info.number_of_seasons}</p>
            <p>Episódios: {info.number_of_episodes}</p></div> : false
            }
